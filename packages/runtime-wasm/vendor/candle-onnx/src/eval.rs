...
fn simple_eval_(
    ...
    for node in graph.node.iter() {
        ...
        match node.op_type.as_str() {
            ...
            // Patched (signal-workbooks): ConvTranspose 2D — used by Depth
            // Anything's decoder for upsampling. Maps the ONNX op to
            // Candle's conv_transpose2d. Symmetric pads only; assumes
            // square kernel/stride/dilation (the only shape we've seen
            // in the wild — generalize when a real model violates it).
            "ConvTranspose" => {
                let dilations = get_attr_opt::<[i64]>(node, "dilations")?;
                let groups = get_attr_opt::<i64>(node, "group")?.copied().unwrap_or(1);
                let _kernel_shape = get_attr_opt::<[i64]>(node, "kernel_shape")?;
                ...
                match auto_pad {
                    None | Some("NOTSET") => (),
                    Some(s) => bail!("unsupported auto_pad for ConvTranspose: {s}"),
                };
                if groups != 1 {
                    bail!("groups != 1 not yet supported in ConvTranspose ({} groups)", groups);
                }
                let xs = get(&node.input[0])?;
                let ws = get(&node.input[1])?;
                if ws.rank() != 4 {
                    bail!("ConvTranspose only supports rank-4 (2D) kernels, got {}", ws.rank());
                }

                let pad_val = match pads {
                    None => 0usize,
                    Some([p]) => *p as usize,
                    Some(&[p1, p2, p3, p4]) => {
                        if p1 != p2 || p1 != p3 || p1 != p4 {
                            bail!("ConvTranspose asymmetric pads not yet supported: {pads:?}");
                        }
                        p1 as usize
                    }
                    Some(p) => bail!("unexpected pads shape in ConvTranspose: {p:?}"),
                };
                let stride_val = match strides {
                    None => 1usize,
                    Some([s]) => *s as usize,
                    Some([s1, s2]) => {
                        if s1 != s2 {
                            bail!("ConvTranspose strides must be equal on both axes: {s1} vs {s2}");
                        }
                        *s1 as usize
                    }
                    Some(s) => bail!("unexpected strides shape in ConvTranspose: {s:?}"),
                };
                let dilation_val = match dilations {
                    None => 1usize,
                    Some([d]) => *d as usize,
                    Some([d1, d2]) => {
                        if d1 != d2 {
                            bail!("ConvTranspose dilations must be equal on both axes: {d1} vs {d2}");
                        }
                        *d1 as usize
                    }
                    Some(d) => bail!("unexpected dilations shape in ConvTranspose: {d:?}"),
                };
                let output_padding_val = match output_padding {
                    None => 0usize,
                    Some([op]) => *op as usize,
                    Some([op1, op2]) => {
                        if op1 != op2 {
                            bail!("ConvTranspose output_padding must be equal on both axes");
                        }
                        *op1 as usize
                    }
                    Some(op) => bail!("unexpected output_padding shape: {op:?}"),
                };

                let ys = xs.conv_transpose2d(
                    ws,
                    pad_val,
                    output_padding_val,
                    stride_val,
                ...
                let ys = if node.input.len() > 2 && !node.input[2].is_empty() {
                    let bs = get(&node.input[2])?;
                    let mut bs_shape = vec![1; ys.rank()];
                    bs_shape[1] = bs.elem_count();
                    ys.broadcast_add(&bs.reshape(bs_shape)?)?
            ...
            "Resize" => {
                ...
                let h = output_dims[2];
                let w = output_dims[3];
                // Patched (signal-workbooks): support linear + cubic resize
                // via Candle's native upsample_bilinear2d. Cubic is
                // approximated as bilinear — Candle has no native bicubic
                // and the visual difference at typical model resize ratios
                // (1.5–4×) is small enough to ship; revisit if a model
                // surfaces obvious banding artifacts.
                //
                // We also relax the asymmetric-only coord-transform check
                // when a non-nearest mode is selected, because bilinear
                // upsample in Candle uses align_corners=false ≈ half_pixel
                // for floats, which is the modern default.
                let output = match mode {
                    "nearest" => {
                        ...
                        if coordinate_transformation_mode != "asymmetric" {
                            bail!(
                                "Unsupported coordinate_transformation_mode for nearest resize: {}",
                    ...
                    "linear" | "cubic" => {
                        // half_pixel / pytorch_half_pixel / align_corners are
                        // the realistic options. Candle's bilinear matches
                        // align_corners=false (≈ half_pixel) closely enough
                        // for inference; mark unsupported variants explicitly
                        // so unexpected modes don't silently produce wrong
                        // output.
