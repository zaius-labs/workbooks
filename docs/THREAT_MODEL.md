...
| 15 | Stolen unlocked recipient laptop with cached lease (A6) | `local_auth::prompt()` in the daemon — macOS Touch ID / Apple Watch / system password via LocalAuthentication framework (objc2 bindings). LAContext.evaluatePolicy with `DeviceOwnerAuthentication` policy; Authorized / Cancelled / Unsupported outcomes; 60s timeout. Returns `Unsupported` on Linux + Windows until proper impls (polkit, Win32 Hello/WebAuthn) land — caller decides fail-closed or fall-through to fresh broker auth. C1.9 lease-cache wires this in as the gate before unwrapping cached DEKs. | ✅ (macOS); ⚠ (Linux + Windows return Unsupported) | C9.5 (closed for macOS); follow-ups for other platforms |
...
| `core-l6n.5` | C9.5 Local-credential gate (cached lease open) | **P0** | ✅ closed (macOS shipped; Linux+Windows = Unsupported, follow-up tickets pending) |
